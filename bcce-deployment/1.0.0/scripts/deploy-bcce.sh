...
if [ "$install_others" = "yes" ]; then
	...
	sed "s/_NAMESPACE_/${k8s_namespace}/g" bcce-cms.yaml.template2 > bcce-cms.yaml
	sed "s/_CMS_PORT_/${cms_port}/g" bcce-cms-svc.yaml.template > bcce-cms-svc.yaml.template1
	sed "s/_NAMESPACE_/${k8s_namespace}/g" bcce-cms-svc.yaml.template1 > bcce-cms-svc.yaml
	rm -f bcce-cms.yaml.template1 bcce-cms.yaml.template2 bcce-cms-svc.yaml.template1

	sudo kubectl apply -f bcce-cms.yaml --namespace=${k8s_namespace}
	sudo kubectl apply -f bcce-cms-svc.yaml --namespace=${k8s_namespace}
	cd ..
	...
	sed "s#_K8S_HOST_#${host_ip}#g" bcce-as.yaml.template1 > bcce-as.yaml.template2
	...
	sed "s/_CMS_PORT_/${cms_port}/g" bcce-as.yaml.template1 > bcce-as.yaml.template2
	sed "s/_BUILD_NUM_/${as_build_num}/g" bcce-as.yaml.template2 > bcce-as.yaml
	...
	rm -f bcce-as.yaml.template1 bcce-as.yaml.template2 bcce-as-svc.yaml.template1

	sudo kubectl apply -f bcce-as.yaml --namespace=${k8s_namespace}
	sudo kubectl apply -f bcce-as-svc.yaml --namespace=${k8s_namespace}
	cd ..
fi
...
if [ "$install_others2" = "yes" ]; then
	...
	sed "s/_BUILD_NUM_/${ps_build_num}/g" bcce-ps.yaml.template2 > bcce-ps.yaml
	rm -f bcce-ps.yaml.template1 bcce-ps.yaml.template2

	sudo kubectl apply -f bcce-ps.yaml --namespace=${k8s_namespace}
	cd ..
	...
	sed "s#_K8S_HOST_#${host_ip}#g" bcce-is.yaml.template1 > bcce-is.yaml.template2
	...
	sed "s/_BUILD_NUM_/${is_build_num}/g" bcce-is.yaml.template1 > bcce-is.yaml
	rm -f bcce-is.yaml.template1 bcce-is.yaml.template2

	sudo kubectl apply -f bcce-is.yaml --namespace=${k8s_namespace}
	cd ..
fi
...
if [ ! -f $intercomp_props ]; then
	...
	sed "s/_BUILD_NUM_/${gs_build_num}/g" bcce-gs.yaml.template2 > bcce-gs.yaml
	...
	rm -f bcce-gs.yaml.template1 bcce-gs.yaml.template2 bcce-gs-svc.yaml.template1

	sudo kubectl apply -f bcce-gs.yaml --namespace=${k8s_namespace}
	sudo kubectl apply -f bcce-gs-svc.yaml --namespace=${k8s_namespace}
	cd ..
fi

echo "##########################################################################################"
echo "Installation of TIBCO BusinessConnect Container Edition is finished now!!!"
echo "##########################################################################################"
echo
