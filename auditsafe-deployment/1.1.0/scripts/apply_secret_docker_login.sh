...
if [ "${docker_repository}" == "" ]; then
	echo "##############################################################################"
	echo "The property docker_repository in the config/deployment.properties isn't set!"
	echo "##############################################################################"
	exit 1
fi
...
if [ "${docker_username}" == "" ]; then
	echo "##############################################################################"
	echo "The property docker_username in the config/deployment.properties isn't set!"
	echo "##############################################################################"
	exit 1
fi
...
kubectl create secret docker-registry ${k8s_key_secret} --namespace=${k8s_namespace} --docker-server=${docker_repository} --docker-username=${docker_username} --docker-password=${password} --dry-run -o yaml | kubectl apply -f -
